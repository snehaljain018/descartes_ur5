...
//  ROS_ERROR_STREAM("Task '"<<__FUNCTION__ <<"' is incomplete. Exiting"); exit(-1);
  ...
  for(unsigned int i = 0; i < poses.size(); i++)
  {
     ...
     *
     */
    descartes_core::TrajectoryPtPtr pt = descartes_core::TrajectoryPtPtr(new descartes_trajectory::AxialSymmetricPt(pose,ORIENTATION_INCREMENT,descartes_trajectory::AxialSymmetricPt::FreeAxis::Z_AXIS));
